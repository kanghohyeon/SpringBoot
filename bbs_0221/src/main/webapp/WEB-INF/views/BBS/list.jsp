									...
									<table summary="게시판 목록">
												...
												<td colspan="4">
													<ol class="paging">
														...
														<c:if test="${requestScope.page ne null }">
															...
															<c:if test="${pvo.startPage >= pvo.pagePerBlock }">
																<li><a
																		href="list&cPage=${pvo.startPage - pvo.pagePerBlock }">&lt;</a>
																</li>
															</c:if>
															<c:forEach begin="${pvo.startPage }" end="${pvo.endPage }"
																...
																<c:if test="${st.index ne pvo.nowPage }">
																	<li>
																		<a href="list&cPage=${st.index }">${st.index
																			}</a>
																	</li>
																</c:if>
															</c:forEach>
										...
										<tbody>
											<c:forEach var="vo" items="${ar }" varStatus="vs">
												<tr>
													<td>${pvo.totalRecord-((pvo.nowPage-1)*pvo.numPerPage+vs.index) }
													</td>
													<td style="text-align: left">
														<a
															href="view?bname=${bname}&b_idx=${vo.b_idx}&cPage=${pvo.nowPage}">
															${vo.subject }
